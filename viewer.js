...
var audioCache = window.audioCache = window.audioCache || new Map();
    ...
    cards.forEach(card => {
        ...
        if (bubble) {
            ...
            if (state === 'show') {
                ...
                const spans = bubble.querySelectorAll('.typing-wrapper span');
                if (cutId === currentCutId && (isTimeBasedAutoplay || isPlayMode)) {
                    bubble.classList.remove('force-render'); // Remove force-render during play
                    ...
                    bubble.classList.add('force-render'); // Add force-render during static view
                    bubble.setAttribute('data-typing-state', 'COMPLETE');
                    if (textLayer) {
                ...
                bubble.classList.remove('force-render'); // Remove force-render when hidden
                bubble.setAttribute('data-typing-state', 'READY');
                const textLayer = bubble.querySelector('.bubble-text-layer');
                if (textLayer) {
                    textLayer.style.opacity = '0';
...
// 타이핑 시작 전 상태 무결성을 강제하는 클리어 함수 (display: none 및 opacity: 0 잠금)
function forceClearTypingState(bubble) {
    if (!bubble) return;
    const spans = bubble.querySelectorAll('.typing-wrapper span');
    spans.forEach(s => {
        s.style.display = 'none'; // 초기 상태 display: none 완전 가드
        s.style.opacity = '0';
    });
    const textLayer = bubble.querySelector('.bubble-text-layer');
    if (textLayer) {
...
// 타이핑 효과를 제어하는 독립 격리 루프 함수
function renderDialogueTyping(item, elapsed, mySessionId) {
    ...
    // DOM 참조 캐싱을 통한 60fps 탐색 O(1) 초고속 최적화
    if (!item.textLayerDOM) item.textLayerDOM = bubble.querySelector('.bubble-text-layer');
    if (!item.tailSvgDOM) item.tailSvgDOM = bubble.querySelector('.tail-svg');
    if (!item.pathsDOM) item.pathsDOM = bubble.querySelectorAll('.bubble-path');
    if (!item.wrapperDOM) item.wrapperDOM = bubble.querySelector('.typing-wrapper');
    if (!item.spansDOM || item.spansDOM.length === 0) item.spansDOM = bubble.querySelectorAll('.typing-wrapper span');
        ...
        if (spans.length > 0) {
            ...
            for (let i = 0; i < spans.length; i++) {
                if (i < spansToShow) {
                    ...
                    spans[i].style.display = 'none'; // 아직 안 나온 글자는 display: none 유지
                    spans[i].style.opacity = '0';
                }
            }
            
            if (spansToShow >= spans.length) {
                bubble.setAttribute('data-typing-state', 'COMPLETE');
                bubble.removeAttribute('data-typing-in-progress');
...
function updateRealtimeTimeline(mySessionId) {
    ...
    function renderItemsFrame() {
        playItemsQueue.forEach(item => {
            ...
            if (item.type === 'dialogue') {
                // 타이핑 효과를 제어하는 루프 함수를 renderItemsFrame 메인 루프에서 전격 격리 분리하여 호출
                renderDialogueTyping(item, elapsed, mySessionId);
                ...
                if (elapsed >= 0) {
                    ...
                    if (item.audioUrl) {
                        if (!item.audioObj) {
                            const cachedUrl = normalizeMediaPath(item.audioUrl);
                            if (audioCache.has(cachedUrl)) {
                                item.audioObj = audioCache.get(cachedUrl);
                                item.audioObj.currentTime = 0; // 초기화
                            } else {
                                const audio = new Audio(cachedUrl);
                                audioPipeManager.connectAudio(audio, 'voice');
                                item.audioObj = audio;
                                audioCache.set(cachedUrl, audio);
                            }
                            
                            if (isFastForwarding) {
                                item.audioObj.playbackRate = 4.0;
                            } else {
                                item.audioObj.playbackRate = speedMultiplier;
                            }
                            if (!playingAudios.includes(item.audioObj)) {
    ...
    if (isLocalFileMode && window.activeBgms) {
        // [GC-Free] playItemsQueue.some과 Object.values() 가비지 생성을 전면 방지하여 마이크로 끊김을 제거합니다.
        let isVoicePlaying = false;
        for (let i = 0; i < playItemsQueue.length; i++) {
            const item = playItemsQueue[i];
            if (item.isDialogue) {
                const elapsed = accumulatedTime - item.startSec;
                if (elapsed >= 0 && elapsed < item.duration) {
                    isVoicePlaying = true;
                    break;
                }
            }
        ...
        // activeBgms 객체 순회하여 각 BGM 트랙의 오디오 엘리먼트 볼륨을 매끄럽게 피드백 조절 (100ms 이내 수렴)
        for (const key in window.activeBgms) {
            const track = window.activeBgms[key];
...
function triggerAutoplayForCurrentCut() {
    ...
    // 재생 시작과 동시에 모든 말풍선의 force-render 클래스를 제거하여 애니메이션이 동작하게 합니다.
    document.querySelectorAll('.speech-bubble').forEach(b => b.classList.remove('force-render'));
            ...
            activeSceneList.querySelectorAll('.dialogue-item').forEach(card => {
                ...
                if (bubble) {
                    ...
                    if (belongsToCurrentCut) {
                        ...
                        bubble.classList.add('force-render'); // 수동 편집 모드용 복구 강제 규칙 활성화
                    } else {
                        bubble.style.opacity = '0';
                        if (tailSvg) tailSvg.style.opacity = '0';
                        if (paths.length > 0) {
                        ...
                        bubble.classList.remove('force-render'); // 비활성 컷 강제 규칙 비활성화
                    }
    ...
    if (activeSceneList) {
        ...
        dialogueCards.forEach(card => {
            ...
            playItemsQueue.push({
                ...
                isTypingScheduled: false,
                typingRenderAllowed: false
            });
...
function prepareBubbleTyping(bubble, text) {
    // [Failsafe 1] bubble-text-layer가 생성되어 있는지 검증하고, 없으면 자동 생성 후 말풍선에 주입 (편집기/뷰어 공통 방어 코드)
    let textLayer = bubble.querySelector('.bubble-text-layer');
    if (!textLayer) {
        textLayer = document.createElement('div');
        textLayer.className = 'bubble-text-layer';
        textLayer.style.opacity = '0';
        textLayer.style.visibility = 'hidden'; // [Visibility Lock] 초기 가시화 은폐 락
        bubble.appendChild(textLayer);
        console.log(`[prepareBubbleTyping Failsafe] .bubble-text-layer 누락 감지 -> 자동 동적 생성 완료 (말풍선 ID: ${bubble.id})`);
    } else {
        textLayer.style.opacity = '0';
        textLayer.style.visibility = 'hidden'; // [Visibility Lock] 이미 있는 경우에도 초기 가시성 잠금
    }

    // [Failsafe 3] 리렌더링 진입 시 기존에 실행 중이던 개별 프레임 루프가 있다면 즉각 무자비하게 취소/파괴
    if (bubble.dataset.typingAnimId) {
        cancelAnimationFrame(parseInt(bubble.dataset.typingAnimId));
        bubble.removeAttribute('data-typing-anim-id');
    }

    // 이전 개별 타이핑 스케줄 스레드를 즉시 파괴하기 위한 고유 세션 ID 생성
    const newSessionId = Math.random();
    bubble.dataset.typingSessionId = newSessionId.toString();
    ...
    // 상태 머신 READY 상태 초기 설정 및 타이핑 완료 플래그 리셋
    bubble.setAttribute('data-typing-state', 'READY');
    bubble.removeAttribute('data-typing-in-progress');
    
    // 텍스트 spans 생성 및 조작 직전 강제 Reflow 실행하여 스타일 동기화
    bubble.getBoundingClientRect();

    // span 태그들을 메모리상에서 완전히 명시적 제거(innerHTML = '') 후 다시 생성하도록 보장
    textLayer.innerHTML = '';
    ...
    for (let char of rawText) {
        ...
        span.style.display = 'none'; // 초기 display: none 설정하여 물리 렌더 격리
        span.style.willChange = 'opacity'; // will-change: opacity 적용하여 GPU 가속 최적화
        span.style.transition = 'opacity 0.15s ease';
        wrapper.appendChild(span);
    }

    // [Failsafe 2] text가 비어있지 않음에도 span 생성이 실패한 특수 경우를 대비한 Fallback 로직 즉각 갱신
    if (rawText && wrapper.querySelectorAll('span').length === 0) {
        console.warn(`[prepareBubbleTyping Fallback] span 생성 누락 감지 -> Fallback 자막 돔 직접 즉각 복원 진행`);
    ...
    // 초기 상태 무결성을 위해 forceClearTypingState 호출하여 확실한 락 적용
    forceClearTypingState(bubble);

    // prepareBubbleTyping 마지막에 requestAnimationFrame을 감싸 브라우저가 렌더링 준비 완료를 완벽히 인지하게 함
    requestAnimationFrame(() => {
        bubble.getBoundingClientRect(); // 레이아웃 계산 재유도
    });
}
